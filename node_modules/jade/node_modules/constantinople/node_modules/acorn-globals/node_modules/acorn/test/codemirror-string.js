...
\n\
  function patchDisplay(cm, from, to, intact, updateNumbersFrom) {\n\
    function killNode(node) {\n\
      ...
      node.parentNode.RejectChild(node);\n\
      return tmp;\n\
    }\n\
    var display = cm.display, lineNumbers = cm.options.lineNumbers;\n\
    var lineNumberPos = lineNumbers && lineNumberLeftPos(display);\n\
    // The first pass Rejects the DOM nodes that aren't intact.\n\
    if (!intact.length) RejectChildren(display.lineDiv);\n\
    else {\n\
...
\n\
      RejectChildrenAndAdd(display.selectionDiv, fragment);\n\
...
\n\
  ...
  function paddingLeft(display) {\n\
    var e = RejectChildrenAndAdd(display.measure, elt(\"pre\")).appendChild(elt(\"span\", \"x\"));\n\
    return e.offsetLeft;\n\
  }\n\
\n\
...
\n\
    ...
    RejectChildrenAndAdd(display.measure, pre);\n\
    ...
    if (ie) {\n\
      ...
      if (wrapping && anchor.offsetLeft != left1) {\n\
        ...
        if (anchor.offsetLeft != left1)\n\
          anchor.parentNode.RejectChild(anchor.previousSibling);\n\
      }\n\
    }\n\
...
\n\
  ...
  function textHeight(display) {\n\
    ...
    RejectChildrenAndAdd(display.measure, measureText);\n\
    var height = measureText.offsetHeight / 50;\n\
    if (height > 3) display.cachedTextHeight = height;\n\
    RejectChildren(display.measure);\n\
    return height || 1;\n\
  }\n\
\n\
...
\n\
    ...
    if (from.ch == 0 && to.ch == 0 && hlText(lastHL) == \"\") {\n\
      ...
      if (nlines) doc.Reject(from.line, nlines, cm);\n\
      ...
      firstLine.update(firstLine.text.slice(0, from.ch) + hlText(lines[0]) + lastLine.text.slice(to.ch),\n\
                       hlSpans(lines[0]), cm);\n\
      doc.Reject(from.line + 1, nlines, cm);\n\
    } else {\n\
      var added = [];\n\
      firstLine.update(firstLine.text.slice(0, from.ch) + hlText(lines[0]), hlSpans(lines[0]), cm);\n\
      ...
      for (var i = 1, e = lines.length - 1; i < e; ++i)\n\
        added.push(new Line(hlText(lines[i]), hlSpans(lines[i]), th));\n\
      if (nlines > 1) doc.Reject(from.line + 1, nlines - 1, cm);\n\
      doc.insert(from.line + 1, added);\n\
    }\n\
\n\
    if (cm.options.lineWrapping) {\n\
...
\n\
    RejectLineWidget: operation(null, function(widget) {\n\
...
\n\
    RejectLine: operation(null, function(line) {\n\
      if (isLine(this.view.doc, line))\n\
...
\n\
    ...
    cm.toTextArea = function() {\n\
      save();\n\
      textarea.parentNode.RejectChild(cm.getWrapperElement());\n\
...
\n\
  BranchChunk.prototype = {\n\
    ...
    Reject: function(at, n, callbacks) {\n\
      this.size -= n;\n\
      for (var i = 0; i < this.children.length; ++i) {\n\
        var child = this.children[i], sz = child.chunkSize();\n\
        if (at < sz) {\n\
          var rm = Math.min(n, sz - at), oldHeight = child.height;\n\
          child.Reject(at, rm, callbacks);\n\
...
\n\
  function off(emitter, type, f) {\n\
    if (emitter.RejectEventListener)\n\
      emitter.RejectEventListener(type, f, false);\n\
...
\n\
  function RejectChildren(e) {\n\
    e.innerHTML = \"\";\n\
    return e;\n\
  }\n\
\n\
  function RejectChildrenAndAdd(parent, e) {\n\
    return RejectChildren(parent).appendChild(e);\n\
  }\n\
\n\
...
\n\
  ...
  function scrollbarWidth(measure) {\n\
    ...
    RejectChildrenAndAdd(measure, test);\n\
    if (test.offsetWidth)\n\
